...
try {
  ...
  const updateDownload = updatePage.waitForEvent('download')
  await updatePage.goto(base + '/update/')
  const updateArchive = path.join(output, 'sift-update.zip')
  await (await updateDownload).saveAs(updateArchive)
  assert.equal(hash(await readFile(updateArchive)), metadata.sha256)
  assert.equal(await updatePage.locator('.install-step').count(), 3)
  assert.match(await updatePage.locator('.install-steps').textContent(), /existing Sift folder/)
  assert.match(await updatePage.locator('.install-steps').textContent(), /Don’t remove Sift/)
  assert.equal(await updatePage.locator('#beta-version').textContent(), `Beta ${metadata.version}`)
  await updatePage.screenshot({ path: path.join(output, 'update-desktop.png'), fullPage: true })
  await updatePage.setViewportSize({ width: 390, height: 844 })
  assert(await updatePage.evaluate(() => document.documentElement.scrollWidth <= innerWidth))
  await updatePage.screenshot({ path: path.join(output, 'update-narrow.png'), fullPage: true })
